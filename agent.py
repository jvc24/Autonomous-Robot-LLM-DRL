...
class Agent(object):
    ...
    def train(self, env, memory: ReplayBuffer, episodes=1000, batch_size=64, updates_per_step=1,
          summary_writer_name="", max_epsiode_steps=100, use_speed_bonus=True):
        """
        ...
        Args:
            ...
            updates_per_step (int): Number of gradient updates per step
            summary_writer_name (str): TensorBoard run name
            max_epsiode_steps (int): Maximum steps per episode
            use_speed_bonus (bool): If True, apply time-efficiency bonus for faster completion
        """
        ...
        print(f"🚀 Training started. Speed bonus: {'ENABLED' if use_speed_bonus else 'DISABLED'}")
        ...
        for episode in range(episodes):
            ...
            while not done and episode_steps < max_epsiode_steps:
                ...
                # Update policy and critic if memory ready
                ...
                # ---- Reward shaping for training (speed bonus) ----
                if use_speed_bonus and reward > 0:  # success condition
                    max_bonus = 1.0
                    cutoff_steps = 100  # Encourage completing task within 100 steps
                    bonus = max(0, max_bonus * (1 - (episode_steps / cutoff_steps)))
                    bonus = min(bonus, max_bonus)
                    reward += bonus  # Modify reward for training
                else:
                    bonus = 0.0
                ...
                # Store the shaped reward for training
                memory.store_transition(state, action, reward, next_state, mask)
                state = next_state

            # Log metrics
            writer.add_scalar('reward/train', episode_reward, episode)
            print(f"🏁 Episode {episode:3d} | Steps: {episode_steps:3d} | "
                f"Reward: {episode_reward:.2f} | Bonus: {bonus:.2f} | "
                f"Speed Bonus: {'ON' if use_speed_bonus else 'OFF'}")
