...
class Agent(object):
    def __init__(self, num_inputs, action_space, gamma, tau, alpha, target_update_interval,
        ...
        # Different Policy and Critic networks for different goal like microwave
    ...
    def train(self, env, memory: ReplayBuffer, episodes=1000, batch_size=64, updates_per_step=1,
          summary_writer_name="", max_episode_steps=100, use_speed_bonus=True, phase=1):
        """
        Train the agent using SAC with optional time-efficiency (speed) bonus.
        Supports multi-phase training with continuous episode numbering.
        """
        ...
        print(f"\n🚀 Starting Phase {phase} | Speed bonus: {'ENABLED' if use_speed_bonus else 'DISABLED'}")

        # --- Load existing logs (to continue numbering) ---
        os.makedirs("logs", exist_ok=True)
        log_file = "logs/training_stats.json"
        if os.path.exists(log_file):
            with open(log_file, "r") as f:
                existing_logs = json.load(f)
            last_global_episode = existing_logs[-1]["episode"]
        else:
            existing_logs = []
            last_global_episode = -1
        ...
        for episode in range(episodes):
            ...
            while not done and episode_steps < max_episode_steps:
                ...
                # Update networks if memory ready
                if memory.can_sample(batch_size):
                    for _ in range(updates_per_step):
                        c1_loss, c2_loss, p_loss, ent_loss, alpha = self.update_parameters(memory, batch_size, updates)
                ...
                if use_speed_bonus and reward > 0:
                    max_bonus = 1.0
                    cutoff_steps = 100
                    bonus = max(0, max_bonus * (1 - (episode_steps / cutoff_steps)))
                    reward += min(bonus, max_bonus)
                else:
                    bonus = 0.0
            ...
            # --- Logging ---
            global_episode = last_global_episode + episode + 1
            writer.add_scalar('reward/train', episode_reward, global_episode)
            print(f"🏁 Phase {phase} | Ep {global_episode:3d} | Steps: {episode_steps:3d} | "
                f"Reward: {episode_reward:.2f} | Bonus: {bonus:.2f}")
            ...
            # --- Save checkpoint & logs every 10 episodes ---
            if (episode + 1) % 10 == 0 or episode == episodes - 1:
                self.save_checkpoint()
                existing_logs.extend(self.training_logs)
                with open(log_file, "w") as f:
                    json.dump(existing_logs, f, indent=4)
                print(f"💾 Saved logs up to episode {existing_logs[-1]['episode']} (Phase {phase})")
                self.training_logs = []

        print(f"✅ Phase {phase} completed — last episode: {existing_logs[-1]['episode']}")
    ...
    def test(self, env : RoboGymObservationWrapper, episodes=1, max_episode_steps=500, prev_action=None):
        ...
        for episode in range(episodes):
            ...
            return prev_action, episode_reward, episode_steps
