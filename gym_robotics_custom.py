...
class RoboGymObservationWrapper(ObservationWrapper):
    ...
    def __init__(self, env: gym.Env, goal='microwave'):
        ...
        env_model.opt.gravity[:] = [0, 0, -1] #Custom Gravity reduce for easire sim
        self.goal = goal
    ...
    def process_observation(self, observation):
        ...
        if self.goal not in obs_achieved_goal:
            raise ValueError(
                f"Goal '{self.goal}' not found in achieved_goal keys: {obs_achieved_goal.keys()}"
            )

        obs_concatenated = np.concatenate((
            obs_pos,
            obs_achieved_goal[self.goal],
            obs_desired_goal[self.goal]
        ))
